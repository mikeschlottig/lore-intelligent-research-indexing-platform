...
export async function executeTool(name: string, args: any, context: ToolContext): Promise<ToolResult> {
  try {
    switch (name) {
      case 'tavily_search': {
        if (!context.tavilyKey) return { error: 'Tavily API key is missing. Please configure it in Settings.' };
      ...
      case 'exa_search': {
        if (!context.exaKey) return { error: 'Exa API key is missing. Please configure it in Settings.' };
        const response = await fetch('https://api.exa.ai/search', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'x-api-key': context.exaKey
          },
          body: JSON.stringify({
            query: args.query,
            useAutoprompt: true,
