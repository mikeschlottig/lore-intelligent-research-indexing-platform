...
import { Trash2, Plus, Server, Key, Settings2, Activity, ShieldCheck, AlertTriangle } from 'lucide-react';
      ...
      <DialogContent className="max-w-2xl bg-paper border-4 border-ink shadow-sketch-lg p-0 overflow-hidden">
        <div className="bg-ink text-paper p-6 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Settings2 className="size-6 text-accent-purple" />
            <DialogHeader className="space-y-0">
              <DialogTitle className="serif-heading text-3xl font-bold">Configuration Ledger</DialogTitle>
            </DialogHeader>
          </div>
          <Badge className="bg-accent-purple text-paper handwritten text-lg px-4 border-none">v1.0.4-LORE</Badge>
        </div>
        <div className="p-8 space-y-8 max-h-[70vh] overflow-y-auto">
          {/* System Health Section */}
          <div className="bg-white/50 border-2 border-ink/5 rounded-2xl p-6 space-y-4 shadow-sm">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <Activity className="size-5 text-accent-purple" />
                <h3 className="serif-heading text-xl font-bold">AI Gateway Status</h3>
              </div>
              <Badge variant="outline" className="border-green-500 text-green-600 bg-green-50 gap-1 font-bold">
                <ShieldCheck size={12} /> Connected
              </Badge>
            </div>
            <div className="text-[11px] text-muted-foreground bg-accent-purple/5 p-4 rounded-xl border border-accent-purple/10 space-y-2">
              <p className="font-bold flex items-center gap-2 text-ink">
                <AlertTriangle size={14} className="text-amber-500" /> 
                Developer Configuration Notice
              </p>
              <p>Lore requires your <strong>Cloudflare AI Gateway URL</strong> in <code>wrangler.jsonc</code>. You can find your Account ID and Gateway ID in the Cloudflare Dashboard under AI &gt; AI Gateway.</p>
            </div>
          </div>
          <div className="space-y-6">
            <div className="flex items-center gap-2 border-b border-ink/10 pb-2">
              <Key className="size-5 text-accent-purple" />
              <h3 className="serif-heading text-xl font-bold">Research Tool Keys</h3>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                <Label className="handwritten text-xl">Tavily Search Token</Label>
                <Input
                  type="password"
                  value={keys.tavilyKey}
                  onChange={e => setKeys(prev => ({ ...prev, tavilyKey: e.target.value }))}
                  placeholder="tvly-..."
                  className="bg-white border-2 border-ink/20 focus:border-ink h-11 shadow-sm"
                />
              </div>
              <div className="space-y-2">
                <Label className="handwritten text-xl">Exa Neural Token</Label>
                <Input
                  type="password"
                  value={keys.exaKey}
          ...
          <div className="space-y-6">
            <div className="flex items-center gap-2 border-b border-ink/10 pb-2">
              ...
              <h3 className="serif-heading text-xl font-bold">MCP Extensibility</h3>
            </div>
            <div className="space-y-3">
              {mcpServers.length > 0 ? mcpServers.map((s, i) => (
                ...
                <p className="text-center py-4 handwritten text-lg text-ink/30 italic">No external archives linked.</p>
              )}
            </div>
            <div className="bg-ink/5 p-4 rounded-2xl space-y-4">
              <p className="handwritten text-lg font-bold">Link New Repository</p>
              <div className="grid grid-cols-1 md:grid-cols-5 gap-3 items-end">
                <div className="md:col-span-2 space-y-1">
                  <Label className="text-[10px] uppercase font-bold tracking-widest text-ink/40 pl-1">Name</Label>
                  <Input
                    placeholder="e.g. LocalDocs"
                    value={newMcp.name}
                ...
                <div className="md:col-span-2 space-y-1">
                  <Label className="text-[10px] uppercase font-bold tracking-widest text-ink/40 pl-1">SSE URL</Label>
                  <Input
